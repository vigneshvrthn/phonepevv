...
with st.title("Title of your page"):
    ...
    with cols1[0]:
        st.image(Image.open(r"C:\Users\krish\OneDrive\Desktop\project\phonepe\PhonePe-Logo.png",)) 
    with cols1[1]:
        st.image(r"C:\Users\krish\OneDrive\Desktop\project\phonepe\pulse-a7eb70ef6c6c2c5a9d1b3c3fd66752f5.gif",width=200) 
...
if select_fun=="Home":
    ...
    if select=="Top" and transaction_or_user=="Transaction":
        ...
        df = pd.read_sql_query("SELECT state, pincode, SUM(no_of_user) AS user, SUM(amt) AS amt FROM pincode_transaction GROUP BY state, pincode", conection)
        df2 = pd.read_excel(r"C:\Users\krish\OneDrive\Desktop\project\pincode.xlsx")
        df2=df2[["District","StateName","pincode","Division Name"]]
        df3=pd.merge(df,df2,on="pincode")
        df3=df3.drop_duplicates(subset=("state","pincode","user","amt"),keep="first",inplace=False)
        df4=df3[df3['state']==state]
        fig = px.scatter(df4,x='pincode', y='amt',color='amt', size='amt',size_max=20,hover_data={'pincode': True, 'District': True, 'Division Name': True, 'amt': True},
                 title=f'Top transactions by pincode of the selected {state}',color_continuous_scale='Viridis', height=400,width=550)

        fig2 = px.scatter(df4,x='pincode', y='user',color='user', size='user',size_max=20,hover_data={'pincode': True, 'District': True, 'Division Name': True, 'user': True},
                 title=f'Top transactions by pincode of the selected {state}',color_continuous_scale='Viridis', height=400,width=550)
    ...
    if select=="Top" and transaction_or_user=="User":
        ...
        df = pd.read_sql_query("SELECT state,pincode,sum(no_of_user) as user from pincode_user group by state , pincode", conection)
        df2 = pd.read_excel(r"C:\Users\krish\OneDrive\Desktop\project\pincode.xlsx")
        df2=df2[["District","StateName","pincode","Division Name"]]
        df3=pd.merge(df,df2,on="pincode")
        df3=df3.drop_duplicates(subset=("state","pincode","user"),keep="first",inplace=False)
        df4=df3[df3['state']==state]
    ...
    select=st.selectbox("Select any one of the question.",["1.	What are the brands of mobiles used and give the top three mobile?",
                         ...
                         "11.   The top 10 pincode which have the highest transaction amt and give its division name and district name",
                         "12.   Which state have the highest no of appopens ?"])
    ...
    if select=="10.	Give some state  the number of user is  low?":
        ...
        st.plotly_chart(fig)
    if select=="11.   The top 10 pincode which have the highest transaction amt and give its division name and district name":
        df = pd.read_sql_query("SELECT state, pincode, SUM(no_of_user) AS user, SUM(amt) AS amt FROM pincode_transaction GROUP BY state, pincode", conection)
        df2 = pd.read_excel(r"C:\Users\krish\OneDrive\Desktop\project\pincode.xlsx")
        df2=df2[["District","StateName","pincode","Division Name"]]
        df3=pd.merge(df,df2,on="pincode")
        df3=df3.drop_duplicates(subset=("state","pincode","user","amt"),keep="first",inplace=False)
        df3=df3.sort_values("amt")
        df4=df3[["state","District","Division Name","pincode","amt"]].tail(10)
        df4.index=range(1,len(df4)+1)
        st.write("the ten pincode which having the high transaction amt is",df4)
        fig=px.bar(df4,x="Division Name",y="amt",title="10 state that having  transaction_amt is high by pincode",
                   hover_data={"state":True,"District":True,"Division Name":True,"pincode":True,"amt":True},color="amt"
                   ,color_continuous_scale="rainbow"
                   ,height=500,width=1000)
        st.plotly_chart(fig)
    
    if select=="12.   Which state have the highest no of appopens ?":
        df=pd.read_sql_query("select state,sum(app_opens) as app_open from aggregation_user_detail group by state order by app_open desc limit 10",conection)
        df.index=range(1,len(df)+1)
        st.write(df)
        sca=px.scatter(df,x="state",y="app_open",color="app_open",size='app_open',size_max=20,
                 title='the top ten state which having the highest app_opens ',color_continuous_scale='Viridis', height=400,width=550)
        st.plotly_chart(sca)
